...
public class ST_PuzzleDisplay : MonoBehaviour 
{
	...
	public IEnumerator CheckForComplete()
	{
        ...
        if (Complete)
        {
            ...
            GameObject piece = GameObject.Find("img/Plane");
            GameObject general = GameObject.Find("General");
            general.GetComponent<Firstlevelscenemanager>().count=0;
            general.GetComponent<Firstlevelscenemanager>().changeFinalDoor();
            SceneManager.MoveGameObjectToScene(general, SceneManager.GetSceneByName("1st Floor Scene"));
